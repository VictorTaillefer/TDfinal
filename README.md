# 🛡️ ANSSI Cyber-Enricher & Analyzer
...
> ⚠️ **Important** : Pour générer le fichier principal `donnees_anssi_enrichies.csv` via `main.py`, vous devez impérativement ajouter le dossier `data/` à la racine du projet. Ce dossier doit contenir les sous-dossiers `alertes/`, `Avis/`, `mitre/` et `first/` peuplés de leurs fichiers JSON respectifs.
